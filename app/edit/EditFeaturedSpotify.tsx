...
import { updateFeaturedSpotify } from "@/app/actions/creator";
import { extractSpotifyEmbedUrl, getSpotifyEmbedHeight } from "@/lib/spotify";

export default function EditFeaturedSpotify({
  featuredSpotifyUrl,
  featuredSpotifyTitle,
  featuredSpotifyCaption,
}: {
  featuredSpotifyUrl: string | null;
  featuredSpotifyTitle: string | null;
  featuredSpotifyCaption: string | null;
}) {
  const [url, setUrl] = useState(featuredSpotifyUrl ?? "");
  const [title, setTitle] = useState(featuredSpotifyTitle ?? "");
  const [caption, setCaption] = useState(featuredSpotifyCaption ?? "");
  const [savedUrl, setSavedUrl] = useState(featuredSpotifyUrl ?? "");
  const [savedTitle, setSavedTitle] = useState(featuredSpotifyTitle ?? "");
  const [savedCaption, setSavedCaption] = useState(featuredSpotifyCaption ?? "");
  ...
  const embedHeight = embedUrl ? getSpotifyEmbedHeight(embedUrl) : 352;
  ...
  const handleSave = () => {
    if (url.trim() && !embedUrl) {
      setError("Spotify の URL を正しく入力してください（open.spotify.com のURLに対応）");
      return;
    }
    setError(null);
    startTransition(async () => {
      const res = await updateFeaturedSpotify({
        url: url.trim() || null,
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        />
        <p className="text-xs text-slate-400 mt-1">楽曲・アルバム・プレイリスト・アーティスト・ポッドキャストのURLに対応。空欄で削除。</p>
      ...
      {embedUrl && (
        <div className="rounded-xl overflow-hidden relative" style={{ height: embedHeight }}>
          ...
          <iframe
            ...
            style={{ borderRadius: 12, opacity: iframeLoaded ? 1 : 0, transition: "opacity 0.3s" }}
            title="Spotify プレビュー"
      ...
      {!url.trim() && (title.trim() || caption.trim()) && (
        <p className="text-xs text-center text-amber-500 font-semibold">Spotify の URL が設定されていないと、タイトル・文章は公開されません</p>
      )}
      {hasChanges && !saved && (
